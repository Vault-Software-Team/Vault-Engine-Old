...
uniform int shadow_cubemap_set;
...
vec4 spotLight(SpotLight light) {
    ...
    if(isTex == 1) {
        return (mix(texture(texture_diffuse0, texCoords), reflectedColor, metallic) * baseColor * (diffuse * (1.0f - shadow) * inten + (vec4(ambient_color, 1) * ambient)) + specularTexture * ((specular * (1.0f - shadow) * inten) * vec4(light.color, 1)))  * vec4(light.color, 1);
    } else {
        return (mix(baseColor, reflectedColor, metallic) * (diffuse * (1.0f - shadow) * inten  + (vec4(ambient_color, 1) * ambient)) * ((specular * (1.0f - shadow) * inten  + (vec4(ambient_color, 1) * ambient)) * vec4(light.color, 1))) * vec4(light.color, 1);
    }
}
