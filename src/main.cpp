    ...
    std::function<void(uint32_t &PPT, uint32_t &PPFBO, uint32_t &gui_gui)> GUI_EXP =
            [&](uint32_t &PPT, uint32_t &PPFBO, uint32_t &gui_gui) {
                ...
                if (ImGui::Begin(ICON_FA_GAMEPAD " Scene", nullptr,
                                    ...
                                    if (HyperAPI::isRunning) {
                                        ...
                                        if (selectedObject->HasComponent<Rigidbody3D>()) {
                                            ...
                                            body->getWorldTransform().setRotation(
                                                    btQuaternion(rotation.x, rotation.y, rotation.z));
                                        }
                                    }
                                    break;
                                }
                                    ...
                                    if (HyperAPI::isRunning) {
                                        if (selectedObject->HasComponent<Rigidbody2D>()) {
                                            ...
                                            body->SetTransform(body->GetPosition(),
                                                               body->GetAngle() + glm::radians(deltaRot.z));
                                        }
                                        ...
                                        if (selectedObject->HasComponent<Rigidbody3D>()) {
                                            ...
                                            rot.setEulerZYX(glm::radians(deltaRot.z), glm::radians(deltaRot.y),
                    ...
                    if (!HyperAPI::isRunning) {
                        ImGui::SetCursorPosX((HyperAPI::isStopped ? (ImGui::GetWindowSize().x / 2) - (32 / 2) :
                                              (ImGui::GetWindowSize().x / 2) - (32 / 2) - (32 / 2) - 5));
                        ImGui::SetCursorPosY(28);

                        ImVec4 buttonColor = ImGui::GetStyle().Colors[ImGuiCol_Button];
                        ImGui::PushStyleColor(ImGuiCol_Button,
                                              ImVec4(buttonColor.x, buttonColor.y, buttonColor.z, 0.7f));
                    ...
                    if (Scene::m_Object != nullptr && !Scene::LoadingScene) {
                        ...
                        if (Scene::m_Object->HasComponent<Transform>()) {
                            ...
                            if (Scene::m_Object->HasComponent<Rigidbody3D>() && HyperAPI::isRunning) {
                                ...
                                btRigidBody *body = (btRigidBody *) rigidbody.body;
                                body->getWorldTransform().setOrigin(
                                        btVector3(comp.position.x, comp.position.y, comp.position.z));
                            }
...
//    Text::Font font("assets/fonts/OpenSans-Bold.ttf", 48);
    ...
    app.Run([&](uint32_t &shadowMapTex) {
        ...
        camera->updateMatrix(camera->cam_fov, camera->cam_near, camera->cam_far,
                             Vector2(app.winWidth, app.winHeight),
                             winSize);
